...
public class EvidenceCollectNode implements NodeAction {
    ...
    public EvidenceCollectNode(AnalysisAgent analysisAgent, ToolAgent toolAgent) {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        if ("紧急".equals(alarmLevel) || "重要".equals(alarmLevel) || "DIAGNOSIS".equals(intent)) {
            ...
            result = analysisAgent.create().call(input).getText();
        } else {
            logger.info("使用ToolAgent进行轻量查询");
            result = toolAgent.create().call(input).getText();
        }
