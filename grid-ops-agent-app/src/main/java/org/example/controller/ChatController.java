...
public class ChatController {
    ...
    @Autowired
    private GraphStreamService graphStreamService;
    ...
    /**
     * Graph 流式对话 — 使用 CompiledGraph.stream() 实时输出每个节点的执行结果。
     * 与 /api/chat 的区别：每个 Graph 节点完成时立即推送，而非等全部结束才返回。
     */
    @PostMapping(value = "/stream-graph", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public SseEmitter chatStreamGraph(@RequestBody Map<String, String> request) {
        String sessionId = request.getOrDefault("sessionId", UUID.randomUUID().toString());
        String question = request.get("question");
        String userId = request.getOrDefault("userId", "default");

        logger.info("Graph流式对话: sessionId={}, question={}", sessionId, question);
        ...
        sseExecutor.execute(() -> {
            try {
                ...
                graphStreamService.streamGraph(
                    question, sessionId, userId, null, // intent=null 由 Router 自动判定
                    new GraphStreamService.StreamCallback() {
                        ...
                        public void onStart() {
                            try {
                                ...
                                logger.warn("发送状态事件失败", e);
                            }
                        }

                        @Override
                        public void onNodeOutput(Map<String, Object> stepInfo) {
                            try {
                                String nodeName = (String) stepInfo.getOrDefault("node", "unknown");
                        ...
                        public void onComplete(String finalResponse) {
                            try {
                                fullAnswer.append(finalResponse);

                                history.add(Map.of("role", "user", "content", question));
                                history.add(Map.of("role", "assistant", "content", finalResponse));

                                saveMessageToDb(sessionId, userId, "user", question, null, null);
                                saveMessageToDb(sessionId, userId, "assistant", finalResponse, null, null);
