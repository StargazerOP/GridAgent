...
public class AlarmController {
    ...
    public SseEmitter diagnoseAlarm(@RequestBody Map<String, String> request) {
        ...
        logger.info("启动告警诊断(流式): taskId={}, description={}", taskId, alarmDescription);
                ...
                graphStreamService.streamGraph(
                    ...
                    new GraphStreamService.StreamCallback() {
                        ...
                        public void onNodeOutput(Map<String, Object> stepInfo) {
                            try {
                                ...
                                emitter.send(SseEmitter.event()
                                        .name("step")
                                        .data(stepInfo));
                                logger.info("SSE 发送节点输出: node={}", nodeName);
                            } catch (IOException e) {
                                logger.warn("发送节点事件失败: node={}", stepInfo.get("node"), e);
                            }
                        }
                        ...
                        public void onError(Exception e) {
                            try {
                                if (alarmTask != null) {
                                    alarmTask.setStatus("FAILED");
                                    alarmTask.setErrorMessage(e.getMessage());
                                    alarmTask.setUpdatedAt(LocalDateTime.now());
                                    alarmTaskMapper.updateById(alarmTask);
                                }
                                emitter.send(SseEmitter.event()
                                        .name("error")
                                        .data("诊断失败: " + e.getMessage()));
